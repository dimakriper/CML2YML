...
def collect_data(list_of_pairs, catalog_data):
    ...
    def parse_pairs(OFFERS_XML, IMPORT_XML):
        ...
        for product in products:
            if check_offerpic(product) and check_description(product):
                ...
                try:
                    ...
                    logger.error(
                        '%s in %s raised an error' % (product_set[offerId].offerId, os.path.split(IMPORT_XML)[1]))
                    raise
                products_received += 1
            else:
                products_declined += 1
        ...
        offer_ids_found_in_offers = []
        not_found_in_offers = 0

        for offer in offers:
            offerId = offer.find('{urn:1C.ru:commerceml_2}Ид').text
            try:
                if offerId in product_set:
                    offer_ids_found_in_offers.append(offerId)
                    ...
                    assert product_set[offerId].quantity != ''
                ...
                logger.error('%s in %s raised an error' % (offerId, os.path.split(OFFERS_XML)[1]))
                raise

        product_set_keys = product_set.copy().keys()
        for key in product_set_keys:
            if key not in offer_ids_found_in_offers:
                not_found_in_offers += 1
                products_received -= 1
                del product_set[key]

        logger.info(
            f'{products_declined} offers declined from {os.path.split(IMPORT_XML)[1]},'
            f' {not_found_in_offers} offers not found in {os.path.split(OFFERS_XML)[1]},'
            f' {products_received} - received')
